    ...
    <div className={`${className?className:' '} mb-2 grid grid-cols-5 gap-2`}>
      {
        new Array(5).fill(0).map((char, i)=>{
          ...
          return (<div key={'char'+guess[i]+i} className={` size-10 sm:size-12 md:size-12 lg:size-12 rounded-md ${bgColor} font-bold uppercase text-black dark:text-[#fff] flex justify-center items-center text-4xl`}>{guess[i]}</div>)
        })
      }
    </div>
  )
