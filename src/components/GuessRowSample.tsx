    ...
    <div className={`${className?className:' '} mb-2 grid grid-cols-5 gap-2`}>
      {
        new Array(5).fill(0).map((char, i)=>{
          ...
          return (<div key={'char'+guess[i]+i} className={` size-10 sm:size-16 md:size-16 lg:size-20 rounded-md ${bgColor} font-bold uppercase text-black dark:text-[#fff] flex justify-center items-center text-4xl`}>{guess[i]}</div>)
        })
      }
    </div>
  )
